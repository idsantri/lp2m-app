...
async function onSubmit() {
	...
	if (input.value.id) {
		response = await apiUpdate({
			endPoint: `penelitian/laporan/${input.value.id}`,
			data,
			confirm: true,
			notify: true,
			loading: loadingCrud,
		...
		if (response) {
			...
			emit('successEdit', response.laporan);
			emit('successSubmit', response.laporan);
		}
	} else {
		response = await apiPost({
			endPoint: 'penelitian/laporan',
			data,
			notify: true,
			loading: loadingCrud,
		});
		if (response) {
			document.getElementById('btn-close-crud').click();
			emit('successAdd', response.laporan);
			emit('successSubmit', response.laporan);
		}
	}
}
